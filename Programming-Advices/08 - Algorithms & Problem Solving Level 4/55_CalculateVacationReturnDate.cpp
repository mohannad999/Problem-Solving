/*
Write a program to read Vacation Start DateFrom and VactionDays ,
...
stDate VacationDate(stDate Date1, short NumberOfVactionDays)
{

    while ((NumberOfVactionDays) > 0)
    {
        if (!(IsWeakend(DayOfWeekOrder(Date1.Day, Date1.Month, Date1.Year))))
        {
            NumberOfVactionDays--;
    ...
    //IncreaseDateByXDays(2,Date1);
    while (IsWeakend(DayOfWeekOrder(Date1.Day, Date1.Month, Date1.Year)))
     Date1 = IncreaseDateByOneDay(Date1);
...
int main()
{
    ...
    DateTo = VacationDate(DateFrom, 23);
    cout << "Return Date :  " ;
